...
class NameMangler:
    def __init__(self, name, prefix, data):
        self._name      = name
        self._prefix    = prefix
        self._mangling  = data.get("mangling", False)
...
class Resource:
    ...
    @property
    def data(self):
        return list(filter(lambda x: (not isinstance(x, tuple)) or x[1](self._parent), self._parts))

    def __call__(self, *args, **kwargs):
        return list(filter(lambda x: (not isinstance(x, tuple)) or x[1](*args), self._parts))
...
class Resources:
    ...
    def function_prefix(self):
        return Resource(self._parent,
                         ResourcePart("template<typename T = {type}>", lambda x: not x.is_dispatched),
                         ResourcePart("inline"),
                         ResourcePart("constexpr"),
                         ResourcePart("friend", lambda x: not x.is_constructor))

    @property
    def function_suffix(self):
        return Resource(self._parent,
                         ResourcePart("const", lambda x: not x.is_member),
                         ResourcePart("noexcept"))
...
class Module(ABC, NameMangler, TypeInfo, Resources):
    def __init__(self, parent, name, data):
        super(Module, self).__init__(data)
        super(Module, self).__init__(self.type, name, data)
...
class InitializerModule(Module):
    def __init__(self, parent, name, data):
        super(InitializerModule, self).__init__(parent, name, data)
        ...
        # build constructors
        self.functions = [Constructor(self, entry, data) for entry in constructors]
...
class BValModule(Module):
    ...
    @name.setter
    def name(self, value):
        self._name = Util.remove_prefix(value, self.module_prefix)
...
class Traits:
    ...
    @property
    def all(self):
        return chain(self.initializer, self.boolean, self.internal, self.external)

class Function(NameMangler, TypeInfo):
    def __init__(self, parent, name, data):
        ...
        self._func_template       = "{prefix} {returns} {name}({args}) {suffix} noexcept"
        ...
        self._prefix     = data.get("prefix", self._resources.function_prefix())

        self._suffix     = data.get("suffix", self._resources.function_suffix())

        returntype = parent.returntype #("b" if (self.parent.name in ["equatable", "comparable", "logical"]) else "z") + self.parent.parent.type
        self._returns    = data.get("returns", "{}<base_t::features>".format(returntype))
        ...
        super(Function, self).__init__(name, self.prefix, data)
    ...
    def signature(self):
        ...
        return self._func_template.format(prefix     = self._prefix,
    ...
    def targets(self):
        if not self.is_dispatched:
            ...
            for (target, body) in self._body.items():
                yield self.new_target(target, body)


    def new_target(self, target, body):
        ...
        return Function(self._parent, pp.name, pp.__dict__)
...
class Constructor(Function):
    def __init__(self, parent, name, data):
        Function.__init__(self, parent, name, data)
...
class Type:
    def __init__(self, config, test_config):
        ...
        self.modules = { m.name:m for m in [Module(self, name, data) for (name, data) in self.traits.all]}#.boolean + self.traits.internal + self.traits.external]}
